...
public class AuthService {
    ...
    public BaseResponse<String> kakaoLogin(String code) {
        ...
        KakaoUserInfoResponseDTO kakaoUserInfo = kaKaoOAuthService.getUserInfo(oauthAccessToken);

        //2. 카카오 이메일로 DB에서 회원 조회
        long kakaoUserId = kakaoUserInfo.getId();
        Member member = memberRepository.findById(kakaoUserId)
                .orElseGet(() -> {
                    // 회원가입 처리
                    Member newMember = new Member();
                    newMember.setEmail("test@example.com");
                    newMember.setName(kakaoUserInfo.getKakaoAccount().getProfile().getNickname());
